...
Only request clarification if absolutely necessary - if the schema and business rules provide sufficient context, proceed without clarification. Return ONLY the JSON."""
    ...
    def _plan_node(self, state: AgentState) -> Dict:
        ...
        with tracer.start_as_current_span("agentic_text2sql.plan") as span:
            ...
            print(f"\n🔍 DEBUG _plan_node: iteration={iteration}")
            print(f"   schema: {state['schema'] is not None}")
            print(f"   metadata_context: {bool(state['metadata_context'])}")
            print(f"   sql_query: {state['sql_query'] is not None}")
            print(f"   execution_result: {state['execution_result'] is not None}")
            print(f"   validation_result: {state['validation_result'] is not None}")
            ...
            if iteration > state["max_iterations"]:
                ...
                print(f"   ➡️  next_action: complete (max iterations)")
                return updates
            ...
            if state["schema"] is None:
                ...
                print(f"   ➡️  next_action: get_schema")
                return updates

            # If no metadata context AND we haven't tried yet, search for relevant context
            # metadata_context can be None (not tried) or [] (tried but empty/failed)
            # Only try once - if it's [] it means we already tried and got nothing
            if state["metadata_context"] is None:
                updates["next_action"] = "search_metadata"
                span.set_attribute("next_action", "search_metadata")
                print(f"   ➡️  next_action: search_metadata")
                return updates
            ...
            if state["sql_query"] is None:
                ...
                print(f"   ➡️  next_action: generate_sql")
                return updates
            ...
            if state["execution_result"] is None:
                ...
                print(f"   ➡️  next_action: execute_sql")
                return updates
            ...
            if state["validation_result"] is None:
                ...
                print(f"   ➡️  next_action: validate_results")
                return updates
            ...
            print(f"   ➡️  next_action: complete")
            return updates
    ...
    def _generate_sql_node(self, state: AgentState) -> Dict:
        ...
        with tracer.start_as_current_span("agentic_text2sql.generate_sql") as span:
            print(f"\n🔍 DEBUG _generate_sql_node called")
            print(f"   user_query: {state['user_query']}")
            print(f"   has_schema: {state['schema'] is not None}")
            print(f"   has_sample_data: {bool(state.get('sample_data'))}")
            metadata_ctx = state.get('metadata_context') or []
            print(f"   metadata_count: {len(metadata_ctx)}")
            ...
            metadata_str = "\n".join([
                doc.get("content", "") for doc in metadata_context
            ])
...
Generate a SQL query to answer this question accurately."""
            ...
            try:
                ...
                print(f"   Raw LLM response length: {len(sql_query)}")
                sql = self._extract_sql(sql_query)
                print(f"   Extracted SQL: {sql[:100] if sql else 'None'}...")
                span.set_attribute("generated_sql", sql)
                print(f"   ➡️  Returning sql_query: {sql is not None}")
                return {"sql_query": sql}

            except Exception as e:
                print(f"   ❌ ERROR in _generate_sql_node: {str(e)}")
                span.record_exception(e)
                return {
                    "error": str(e),
                    "is_complete": True
